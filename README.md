...
Плитки-пресеты в «Настройках → Мониторы» рисует **сама macOS и только для панелей, которые считает Retina (4K/5K)**. На **1440p (QHD)** их честно не завести без EDID-override, а на **Apple Silicon** override для внешних мониторов ненадёжен (EDID не отдаётся системой). Поэтому `hidpi` даёт саму возможность HiDPI напрямую, а компактный выбор «Larger Text ↔ Default ↔ More Space» живёт в терминале.
